...
void AFirstPersonCharacter::BeginPlay()
{
	...
	if (Gun_BP == nullptr)
	{
		UE_LOG(LogTemp, Warning, TEXT("Actor: '%s' is missing a Gun Blueprint reference!"), *GetName());
		return;
	}
	SpawnedGun = GetWorld()->SpawnActor<AGun>(Gun_BP);
	SpawnedGun->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
...
void AFirstPersonCharacter::OnFire()
{
	if (SpawnedGun)
	{
		...
		// Try and play a firing animation on our character's body if specified.
		// (eg. recoil)
